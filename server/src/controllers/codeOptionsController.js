...
/**
 * 카테고리별 코드 옵션 조회
 * GET /api/v1/code-options?category=repair_status
 * GET /api/v1/code-options?category=repair_status,repair_problem_type (복수)
 */
const getCodeOptions = async (req, res) => {
  try {
    ...
    const [options] = await sequelize.query(`
      SELECT id, category, code, label, description, sort_order, is_active, metadata
      FROM system_code_options
      WHERE category IN (:categories) ${activeFilter}
...
/**
 * 코드 옵션 추가
 * POST /api/v1/code-options
 */
const createCodeOption = async (req, res) => {
  try {
    ...
    const [result] = await sequelize.query(`
      INSERT INTO system_code_options (category, code, label, description, sort_order, metadata, created_at, updated_at)
      VALUES (:category, :code, :label, :description, :sort_order, :metadata::jsonb, NOW(), NOW())
...
/**
 * 코드 옵션 수정
 * PATCH /api/v1/code-options/:id
 */
const updateCodeOption = async (req, res) => {
  try {
    const { id } = req.params;
...
/**
 * 코드 옵션 삭제
 * DELETE /api/v1/code-options/:id
 */
const deleteCodeOption = async (req, res) => {
  try {
    const { id } = req.params;
    const [result] = await sequelize.query(`
...
module.exports = { getCodeOptions, createCodeOption, updateCodeOption, deleteCodeOption };
